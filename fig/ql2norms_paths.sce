en_orig = fscanfMat("data/ql2norms_0.dat");
...
count = 0;
for num = 0
    en_orig = fscanfMat('data/ql2norms_'+string(num)+'.dat');
    en = en_orig(zero_index:rw_orig(1),:);
    avg = avg+en;
    count = count+1;
end
...
plot(avg(2:$,1),log(avg(2:$,4:$)./avg(2:$,3:$-1))/log(2))
